...
const WhatSvg = ({ size = 17 }: { size?: number }) => (
...
const headingClass = "mb-5 text-[0.7rem] font-semibold uppercase tracking-[0.25em] text-[#2c2c2c]/40";
const linkClass = "text-[#2c2c2c]/60 transition-colors hover:text-[#2c2c2c]";
const socialWrapperClass = "flex items-center gap-4 text-[#2c2c2c]/40";
...
export default function Footer() {
  ...
  const brandBlock = (
    <div className="flex flex-col gap-4">
      ...
      <p className="max-w-md text-sm leading-relaxed text-[#2c2c2c]/60">
        {t("tagline")}
      </p>
    </div>
  );
  ...
  const socialIcons = (size = 17) => (
    <div className={socialWrapperClass}>
      {facebookUrl && (
        ...
        >
          <Facebook size={size} strokeWidth={1.8} />
        </Link>
      )}
      {instagramUrl && (
        <Link
          ...
          rel="noopener noreferrer"
        >
          <Instagram size={size} strokeWidth={1.8} />
        </Link>
      )}
      {whatsappUrl && (
        <Link
          href={whatsappUrl}
          aria-label="WhatsApp Snapshot Hanoi"
          className="transition-colors hover:text-[#2c2c2c]"
          target="_blank"
          rel="noopener noreferrer"
        >
  ...
  const contactBlock = (
    <div className="flex flex-col gap-4">
      <h3 className="font-serif text-xl font-normal text-[#2c2c2c] tracking-wide">
        {tNav("contact")}
      </h3>
  ...
  const homeBlock = (
    <div>
      <h3 className={headingClass}>{tNav("home")}</h3>
      <ul className="flex flex-col gap-3 text-sm">
        <li>
          <Link href={`/${locale}`} className={linkClass}>{tNav("home")}</Link>
        </li>
        <li>
          <Link href={`/${locale}/about`} className={linkClass}>{tNav("about")}</Link>
        </li>
      </ul>
    </div>
  );

  const galleriesBlock = (
    <div>
      <h3 className={headingClass}>{tNav("galleries")}</h3>
      <ul className="flex flex-col gap-3 text-sm">
        <li>
          <Link href={`/${locale}/galleries`} className="text-[#a08c7b] font-medium transition-colors hover:text-[#2c2c2c]">{tNav("galleries")}</Link>
        </li>
        <li>
          <Link href={`/${locale}/packages`} className={linkClass}>{tNav("packages")}</Link>
        </li>
      </ul>
    </div>
  );
  ...
  const blogsBlock = (
    <div>
      <h3 className={headingClass}>{tNav("blogs")}</h3>
      <ul className="flex flex-col gap-3 text-sm">
        <li>
          <Link href={`/${locale}/blogs`} className={linkClass}>{tNav("blogs")}</Link>
        </li>
      </ul>
    </div>
  );
  ...
  return (
    <footer className="bg-[#f5f3ef] text-[#2c2c2c]">
      <div className="mx-auto max-w-300 px-8 py-16 md:px-12 xl:px-16">
        {/* === MOBILE + TABLET (1 col → md:2 cols) === */}
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2 md:gap-x-16 md:gap-y-14 xl:hidden">
          {/* Row 1, Col 1: Brand */}
          <div className="flex flex-col gap-4">
            {brandBlock}
            <div className="mt-1">{socialIcons(17)}</div>
          </div>

          {/* Row 1, Col 2: Contact */}
          {contactBlock}

          {/* Row 2, Col 1: Home links */}
          {homeBlock}

          {/* Row 2, Col 2: Galleries + Blogs */}
        ...
        <div className="hidden xl:grid xl:grid-cols-[1.35fr_0.85fr_0.95fr_0.75fr_1.65fr] xl:gap-10">
          {brandBlock}
          {homeBlock}
          {galleriesBlock}
          {blogsBlock}
      ...
      <div className="border-t border-[#2c2c2c]/10">
        <div className="mx-auto flex max-w-300 flex-col gap-4 px-8 py-5 md:px-12 xl:flex-row xl:items-center xl:justify-between xl:gap-6 xl:px-16">
          {/* Row 1: language + copyright */}
          <div className="flex flex-wrap items-center gap-4 text-sm text-[#2c2c2c]/40">
              ...
              <div className="absolute left-0 mt-2 min-w-28 rounded bg-white shadow-lg z-10 hidden group-hover:block">
                {languages.map((lang) => (
                  <button
                    ...
                    className={`block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 ${locale === lang.code ? "font-bold" : ""}`}
                  >
                    {lang.label}
                  </button>
                ))}
